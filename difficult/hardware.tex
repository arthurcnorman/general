...
An enormous amount could be said about hardware, with such diverse issues
as silicon and device physics, the technology of potential quantum computers,
the practicalities of avoiding things overheating as well as all the
variations on instruction set design. The first issue addressed here
illustrates how something that can seem really trivial in fact conceals
multiple layers of complication. That is followed by tracing enough of the
early history of computing engines to point out that at least the earlier
ones are at least conceptually reasonably straightforward and while
building a system based on those ideas would be a serious size project
it is far from unimaginable.
}\end{quotation}
\section{USB}
