...
The rules for incrementing a number in binary are built around a
transformation where a final string of the form \verb@01*@ (ie a zero followed
by a (possible) string of ones) turns into a one (possibly) followed by
zeros. The word ``possible`` is being used there to ensure that the case that
the last significant bit of the number starts off as a 1 is coped with.
All bits to the left of that final zero must remain unchanged.
A pattern line \verb@$(0|1)*01*$@ spots this within a single number, with the
fences \verv@$@ marking that the start and end of the number. That
idea can be built on with something like \verb@$(0|1)*01*$(0|1)*10*$@
that attempts to starting to show how the low bits in must relate from one
digit to the next. This still has two big issues: the position within
it of everything is not well enough tied down, and there is nothing yet to
insist that high order bite remain unchanged.
