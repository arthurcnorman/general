...
$P Q$, $P | Q$, $P \& Q$ and $P*$. The one for $\sim P$ is especially
...
representing it in more compact ways than just a simple big rectangular
...
problem is that ``long enough'' might hugely exceed the expected lifetime
of the universe.
...
that measures out lengths of strings. It is really easy
to set up a first ruler -- say one that marks out blocks of size 3.
can be just
\begin{verbatim}
   ruler3    ...
\end{verbatim}
where the core of the ruler is just the string of three unspecified
characters.

The step on from this is to set up an expression that will match patterns
that when this ruler is of length $k$ they are of length $2^k$. A path
towards this is to seek a combination of patterns that will only match things
that are basically like
\begin{verbatim}
   $000$001$010$011$100$101$110$111$
\end{verbatim}
ib other words a sequnce of ruler-length segments that count in binary, and
that use some fence charecter to separate them. This is not as terrible
a challenge as it may at first seem: we set up regular expressions one
at a time each of which forces some partial constraint on what is allowed
and combine them all with \verb@&@ until the above is the only possibility
left. Well actually it is a little more fancy than that. A first step
is to impose the basic regularity and \verb@($ {ruler3})* $@ is a pattern that
can generate the upper line in the following.
\begin{verbatim}
   $...$...$...$...$...$...$...$...$
   $000$001$010$011$100$101$110$111$
\end{verbatim}
We can now view pairs of characters that are positioned one above each other
as single units in a larger alphabet. In running text I will show these
pairs in angle brackets> and with that it becomes easy to force all the
\verb@$@ characters in the two lines to match up:
\begin{verbatim}
   (<$$> | <~$~$>)*
\end{verbatim}
since this is just intended to say that any \verb@$@ in one line must
match with one on the other.
Next it is easy to force the start of the text to begin with \verb@$000$@:
\begin{verbatim}
   <$$>(.0)*<$$>.*
\end{verbatim}
reads that between the first two fences while anything goes on the top line
the bottom line is all zeros. The final \verb@.*@ says that as far as this
little rule is concerned anything goes after that first segment.

Now for the bit which is obviously going to be tricky which arranges that
each segment can be read as the next binary number up from the previous one.
The process of incrementing a binary number can be achieved using the
following constraints, only provided that the underlined characters can be
kept exactly one ruler-length apart:\\
...
where the first two cases have at least one zero after the particular
character and the second two do not -- which will mean that any characters
there will all be ones. Note that these four cases are mutually exclusive but
also cover every possibility in the part before the \verb@$@ fence mark.
If we can set up patterns for each then just the ``or'' of all those
pattens will cover all cases and will force the segment to the right
of the fence to be represent the next number up. Well that insight is
not quite right yet because the patterns shown must apply in every bit
position while simple us of the above patterns do not ensure that. The
fix is to adjust each of the four patterns to match an incorrect
bit in the subsequent block, as in\\
\verb@       ...@\textbf{\underline{0}}\verb@ ... 0 ... $ ... @\textbf{\underline{1}}\verb@ ...@\\
and in that way set up a pattern that will successfully match any sequence
that is {\em not} a valid counting sequence. The negation of this will then
capture just the ones that are correct.

Arranging that the underlined characters are exactly the correct distance
apart can use basically the same scheme as had been used to force
everything into regular blocks -- basically it views the underlining as
an separable part of the character and uses a ruler-based pattern rather
like\\
\verb@    .*@\textbf{\underline{ }}\verb@xxxx@\textbf{\underline{ }}\verb@.*@\\
to sset up two underscores that are in this case 4 characters apart. But
the \verb@xxxx@ can be replaced by any longer sequence whose length
is fixed.
