...
does not (thus far) illustrate subtraction and division. A different
track may make those easier to grasp. One of the most basic and hence
...
possible to extract eacg component. If you have pairs it will be possible
to chain them as lists and view the length of the list as an alternate
denotation of a number. If one can concatenate lists that corresponds
to addition, and taking the tail of a list subtracts one. But as well as
using pairs to model numners that can be used to build up vectors and
...
\begin{lstlisting}[mathescape=true]
   ...
   let right $p$ = $p$ false;
\end{lstlisting}
where in this presentation there are some extra parentheses in the
...
fewer arguments than had been shown in the textual definitio is not a
problem. All that happens is that one gets left with an object that
can sit around only partially evaluated awaiting the remaining argument
or arguments.
...
for the benefit of humans not just machines.
...
\begin{lstlisting}[mathescape=true]
   let Y $f$ = $f$ (Y $f$); 
\end{lstlisting}
where of course that precise formulatio not acceptable because \verb+Y+
...
mess of uses of jusr \verb+S+ and \verb+K+ in a perfectly straightforward
way.

We can then exploit $Y$ by arranging that from now onwards any \verb+let+
declaration is expanded a little such that
\begin{lstlisting}[mathescape=true]
...
\end{lstlisting}
where this goes back to using the abstraction notation used when
function definitions were first considered here.
